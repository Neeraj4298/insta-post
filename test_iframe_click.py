...
def click_recaptcha_and_upload():
    print("==================================================")
    print(" Explicit reCAPTCHA Frame Clicker & Reel Upload   ")
    print("==================================================")
    ...
    with sync_playwright() as p:
        ...
        if len(inputs) >= 2:
            ...
            print("Submitted login. Waiting 5s for reCAPTCHA check...")
            time.sleep(5)

        # Scan for reCAPTCHA frame after login submission
        print("Scanning page frames for reCAPTCHA...")
        target_frame = None
        for frame in page.frames:
            print(f"Detected Frame URL: {frame.url}")
            if "recaptcha" in frame.url or "bframe" in frame.url or "anchor" in frame.url:
                target_frame = frame
                break

        if target_frame:
            print(f"✓ Found reCAPTCHA frame: {target_frame.url}")
            anchor = target_frame.locator("#recaptcha-anchor, .recaptcha-checkbox-border, #rc-anchor-container").first
            anchor.wait_for(state="visible", timeout=5000)
            anchor.click()
            print("✓ Explicitly clicked #recaptcha-anchor inside frame!")
            time.sleep(5)
        else:
            print("No reCAPTCHA frame detected. Checking for main frame checkbox...")
            try:
                frame_loc = page.frame_locator("iframe[title*='reCAPTCHA'], iframe[src*='recaptcha']").first
                frame_loc.locator("#recaptcha-anchor, .recaptcha-checkbox-border").first.click()
                print("✓ Clicked reCAPTCHA checkbox via frame_locator!")
                time.sleep(5)
            except Exception as e:
                print(f"Frame locator check result: {e}")

        page.screenshot(path=str(ROOT_DIR / "downloads" / "post_login_captcha.png"))
        print(f"Saved post_login_captcha.png. Page URL: {page.url}")

        # Check if submit button appears post-captcha
        sub = page.locator("button[type='submit'], button:has-text('Submit'), button:has-text('Next')").first
        ...
        page.screenshot(path=str(ROOT_DIR / "downloads" / "final_explicit_result.png"))
        print("\n==================================================")
